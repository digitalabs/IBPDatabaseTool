...
CREATE INDEX changes_idx03 ON changes (cid);
--
...
--
CREATE INDEX georef_idx01 on georef (locid);
     -- do not define locid as primary key..create index instead.
--
...
CREATE INDEX germplsm_idx09 ON germplsm (lgid);
   -- added 20091019 mhabito
CREATE INDEX germplsm_idx10 ON germplsm (gid);
   -- added 20091020 mhabito (in addition to defining GID as unique key)
CREATE INDEX germplsm_idx11 on germplsm (cid);
   -- added 20091216 mhabito
CREATE INDEX germplsm_idx12 on germplsm (sid);
   -- added 20091216 mhabito
...
CREATE INDEX listdata_idx08 ON listdata (listid,lrecid);
 -- added 20091103 mhabito: define regular index on column(s) with UNIQUE KEY constraint
--
...
CREATE INDEX listnms_idx02 ON listnms (listid);
          -- added 20091103 mhabito: define regular index on column(s) with UNIQUE KEY constraint
--
...
CREATE INDEX location_idx05 ON location (locid);
         -- added 20091103 mhabito: define regular index on column(s) with UNIQUE KEY constraint
--
...
CREATE INDEX methods_idx04 ON methods (mid);
		 -- added 20091103 mhabito: define regular index on column(s) with UNIQUE KEY constraint
--
...
CREATE INDEX progntrs_idx04 ON progntrs (gid,pno);
   -- added 20091103 mhabito: define regular index on column(s) with UNIQUE KEY constraint
--
...
CREATE INDEX sndivs_idx02 on sndivs (snlid);
        -- added 20091103 mhabito: define regular index on column(s) with UNIQUE KEY constraint
--
...
CREATE INDEX udflds_idx04 on udflds (fldno);
        -- added 20091103 mhabito: define regular index on column(s) with UNIQUE KEY constraint
--
...
CREATE INDEX users_idx03 on users (userid);
	   -- added 20091103 mhabito: define regular index on column(s) with UNIQUE KEY constraint
--
